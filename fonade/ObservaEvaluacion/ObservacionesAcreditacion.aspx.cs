...
{
    public partial class ObservacionesAcreditacion : Negocio.Base_Page //System.Web.UI.Page
    {
        ...
        private void CargarCombo()
        {
            ...
            ddlPerfil.Items.Insert(1, new ListItem("Emprendedor", "1"));
        ...
        private void cargarConvocatorias()
        {
            ...
            var query = "select Descripcion, ' - ', valor from LOVObjetoSEAcreditacion where NomLovObjetose='Convocatorias'";
        ...
        private int Validar()
        {
            ...
            if (ddlPerfil.SelectedItem.Value == "0")
            {
                return 5;
            }
                    ...
                    if (int.Parse(dt.Rows[0].ItemArray[0].ToString()) == 0)
                    {
                        if (dt.Rows[0].ItemArray[0].ToString() == "0")
                        {
                            ...
                            if (dt.Rows.Count > 0)
                            {
                                ...
                                txtSql = "INSERT INTO ObservacionesAcreditacion (NomObservacionesAcreditacion, codconvocatoria, CodProyecto, NombreConvocatoria, Nombres, Email, Perfil, Comentarios, Fecha) ";
        ...
        private bool EnviarCorreo(DataTable dt, int _codConvocatoria)
        {
            ...
            using (FonadeDBLightDataContext db = new FonadeDBLightDataContext(_cadena))
            {
                emailDestinatario = (from e in db.ObservacionesEvaluacionCorreo
                                     ...
                                     select e.correo_principalAcreditacion).FirstOrDefault();

                emailDestinatarioCC = (from e in db.ObservacionesEvaluacionCorreo
                                       where e.codOperador == codOperador
                                       select e.correo_secundarioAcreditacion).FirstOrDefault();

                emailTerciario = (from e in db.ObservacionesEvaluacionCorreo
                                       where e.codOperador == codOperador
                                       select e.correo_terciarioAcreditacion).FirstOrDefault();
            }
            ...
            try
            {
                ...
                if (emailTerciario != "" && emailTerciario != null && emailTerciario != string.Empty)
                    mail.To.Add(new MailAddress(emailTerciario));
                mail.From = new MailAddress(emailRemitente);
                mail.Subject = "Nueva observación acreditación";
