CREATE   FUNCTION fn_PuntajeEmpleosDirectos (@CodProyecto int, @CodConvocatoria int, @EmpleoEstimado int, @Anio char(4))
returns float
AS 
BEGIN
...
select ((select count(*) as EmpleosDirectos from proyectoinsumo where codtipoinsumo=2 and codproyecto=@codproyecto)+
(select count(*) from proyectogastospersonal where codproyecto=@codproyecto)) as EmpleoDirecto, valorrecomendado 
from proyectometasocial p, evaluacionobservacion e 
where p.codproyecto=e.codproyecto
 and e.codproyecto=@codproyecto
